...
const saveTelegramMediaAsProjectFile = async (
  ...
  const transcription = await transcribeAudioPath({
    path: filePath,
    originalname: media.originalName || originalName,
    filename: fileName,
...
const handleProjectDescription = async (
  ...
  if (!finalDescription) {
    await sendTelegramBotMessage(
      ...
      media && isAudioMediaCandidate(media)
        ? [
            "فایل صوتی دریافت شد اما متن قابل استفاده‌ای از آن استخراج نشد.",
            savedFile?.transcriptionError
              ? `خطای تبدیل صدا به متن: ${escapeTelegramHtml(savedFile.transcriptionError)}`
              : "",
            "لطفاً یک توضیح متنی ارسال کنید یا دوباره با صدای واضح‌تر ویس بفرستید.",
          ]
            .filter(Boolean)
            .join("\n")
        : "توضیح گزارش الزامی است. لطفاً متن توضیح را ارسال کنید یا فایل صوتی را همراه کپشن بفرستید.",
    );
    return;
  }
  ...
  if (savedFile) {
    ...
    await sendTelegramBotMessage(
      getChatId(message),
      "گزارش پروژه همراه با فایل صوتی و متن استخراج‌شده با موفقیت ثبت شد.",
      { replyMarkup: buildMainKeyboard() },
    );
    return;
  }
...
const handleMessage = async (message: TelegramMessagePayload) => {
  ...
  if (text === "/new" || text === "/report" || text === "ثبت گزارش پروژه") {
    await sendProjectSelection(message, user);
    return;
  }
